...
| LOW | 🧹 Delete the verification row `KH-7QA4-D9QM` ("ZZ DEPLOY CHECK (delete me)", `status='created'`, no payment) from Supabase, and ignore the unpaid Razorpay order `order_TeFu2pvrUqqKT0`, which expires on its own | Written deliberately to prove the new column works on production before a real customer met it. Disclosed rather than left to be found. |
