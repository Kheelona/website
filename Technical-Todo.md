...
| Priority | Item | Why |
|---|---|---|
| ~~BLOCKER~~ | ✅ **DONE 2026-09-20.** Migration run on `main PRODUCTION`; a live `create-order` POST answered 200, so PostgREST sees the column and the payment path is intact. | `create-order` inserts `ph_distinct_id` on every order. If the code ships first, PostgREST answers PGRST204 and **EVERY PRE-ORDER RETURNS 500** — the store stops taking money. `0003_fb_attrib.sql`'s own header records this repo nearly doing it once already. **`npm run qa:payment` passing does NOT clear this**: the stub stores whatever it is sent and validates no columns (§8.40-i). Idempotent and safe to re-run. |
...
| ~~LOW~~ | ✅ **DONE 2026-09-20.** Verification row `KH-7QA4-D9QM` deleted by the founder; the guarded `delete … returning` printed exactly one row, so it matched that order and nothing else. The unpaid Razorpay order `order_TeFu2pvrUqqKT0` needs no action and expires by itself. | It was written deliberately to prove the new column worked on production before a real customer met it, and disclosed rather than left to be found. |
...
| Priority | Item | Why |
|---|---|---|
| ~~HIGH~~ | ✅ **DONE 2026-09-20, IN TWO STEPS — and the first one was not enough.** Meta offers no delete for a pixel with history, so every connection was removed instead. **That did NOT silence it**: the pixel kept receiving `PageView` for hours afterwards, because a SECOND, business-level connection was still feeding it (§8.41-i). Closed at 23:32 by excluding dataset `1045085251085243` in **Business Manager → Your Conversions API connections → Manage Datasets**. Datasets connected is now **1**. | Two pixels in the account IS the confusion this round was about; one is the cure. **The lesson: a pixel's own Integrations tab is not the only thing that can feed it.** |
| ~~HIGH~~ | ✅ **DONE 2026-09-20.** Token regenerated and redeployed. 🔴 **Its validity is still unproven** — `capi: "configured"` means PRESENT, not working, and only a real order settles it: look for Purchase with connection method **"Multiple"**, or `[meta-capi] REJECTED` in the Vercel log if it is wrong. | Both tokens shared during this round are in a chat transcript, and they are **business-scoped** — they can write events to any dataset in the portfolio, so deleting the old pixel does NOT neutralise them. Independent of the migration, so do it whenever. **A Vercel variable only applies to deployments created after it changes**, so the Redeploy is not optional. |
| ~~MEDIUM~~ | ✅ **DONE 2026-09-20 23:35.** Full re-verification, both hosts in a real browser plus every Meta-side setting: **Diagnostics reads "No errors at this time."** One pixel initialises on each host, `ViewContent` fires on `/products/kheelu`, the allow list, advanced matching and first-party cookies are on, and all three "automatic" toggles are off. | The single screen that proves the whole round landed. |
| 🔴 **CHECK TOMORROW** | **The old pixel's "last received" must keep ageing and never reset.** Same-session verification was inconclusive and is recorded as such: it did not move across fresh traffic, but the LIVE pixel's own Conversions API card did not register that traffic either, so the instrument had not moved for the control. | If it resets, the exclusion did not hold. **When the instrument has not moved for the control, it proves nothing about the subject.** |
| LOW | **Re-check "Websites" on the new pixel after a day of real traffic.** It reads "No websites found" while the old pixel listed `kheelona.com`. **Not the date filter** — that was tested and disproved, and the filter has since been set to Sep 18–20 with the panel unchanged. Most likely Meta's aggregation not having run for a pixel hours old. Nothing is being dropped: events are accepted, and the allow list would block an unrecognised domain. Worth a real look only if it is still empty tomorrow. | A panel that reads like a fault and is not. |
...
| LOW | **"Automatically connect only your datasets" is ON** (Business Manager → Your Conversions API connections). Any NEW pixel created in this portfolio starts server-side mirroring the moment it exists. | Not a problem today with one pixel. It is how a second dataset would quietly start receiving again. **Cite the page path when naming a Meta setting** — this one's name collides with "Automatically include more detailed page and product info" on the pixel's own Settings page, which is correctly Off (§8.41-i addendum). |
