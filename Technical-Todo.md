...
| Priority | Item | Why |
|---|---|---|
| ~~HIGH~~ | ✅ **DONE 2026-09-20.** Meta offers no delete for a pixel with history, so **every connection was removed instead** — nothing can send to it and no ad account can select it, which is the same outcome. Verified quiet: its last PageView predates the migration deploy. | Two pixels in the account IS the confusion this round was about; one is the cure. |
