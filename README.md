...
- **⚠ REVAMP IN PROGRESS — theme B "Kheelu's Tour" (2026-07-25, branch `revamp/kheelu-tour`, pushed to origin)**: the whole site is being re-themed onto wireframe direction B per the founder brief `Websit prompt based on B + inputs - 24-Jul.pdf`. **READ FIRST on resume: `docs/revamp-2026-07/WORKING.md`** (it opens with a "COLD-RESTART: START HERE" section). Signature = a persistent **Kheelu guide** (fixed corner narrator that tracks the section on screen), content in contained **rooms** on one warm CSS backdrop, no section numbering, brand-4 palette only, two fonts only. DONE: P0 (image hotfix on master), P1 (working file + Gemini hero kit + research memo + full-site copy `docs/revamp-2026-07/copy-v2.md` + QA strategy §8.20), P2 (palette + font law), M1 (theme core: `SiteBackdrop`/`Room`/`RoomsTrack` + `KheeluGuide`), M2 (Home, commit `ddf5996`), M3 (/products/lumi + ColorwayPicker, commit `87a812a`). Home + Lumi visually verified in Chrome; tests **206/206**, build green. **NEXT = M4** (the 8 interior routes on the room grammar), then M5 (delete retired components + full Lighthouse/a11y pass), then P5 (Vercel preview → founder). Founder-gated, never invent: REV-a hero art, REV-b subscription/camera/ship/languages, REV-c stale ₹2,999 index, REV-d app images. Checkpoint: `docs/checkpoints/revamp-theme-b-2026-07-25.md`.
