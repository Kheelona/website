...
- **Locked decisions**: tokens from `Design/design-system/` mapped into `src/styles/globals.css` `@theme` (drift fails the build via `tools/tokens/check-tokens.mjs`, 17 mappings); Tally pre-order (adapter in `src/components/molecules/TallyEmbed.tsx`); ₹ site-wide; A+C blended design (see `docs/checkpoints/phase-2-ux-discovery.md`); accessibility 90+ outranks any styling preference.
