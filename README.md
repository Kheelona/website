...
- **⚠ REVAMP IN PROGRESS — theme B "Kheelu's Tour" (2026-07-25, branch `revamp/kheelu-tour`, pushed to origin; LIVE PREVIEW https://website-hdn2.vercel.app)**: the whole site is re-themed onto wireframe direction B per the founder brief `Websit prompt based on B + inputs - 24-Jul.pdf`. **READ FIRST on resume: `docs/revamp-2026-07/WORKING.md`** (it opens with a "COLD-RESTART: START HERE" section). Signature = a persistent **Kheelu guide** (fixed corner narrator that tracks the section on screen), content in contained **rooms** on one warm CSS backdrop, no section numbering, brand-4 palette only, two fonts only. DONE: P0 (image hotfix on main), P1 (working file + Gemini hero kit + research memo + full-site copy `docs/revamp-2026-07/copy-v2.md` + QA strategy §8.20), P2 (palette + font law), M1 (theme core: `SiteBackdrop`/`Room`/`RoomsTrack` + `KheeluGuide`), M2 (Home, `ddf5996`), M3 (/products/lumi + ColorwayPicker, `87a812a`), **M4 (all 8 interior routes, `7e7e2b7`) — the whole site now runs one theme**, and the preview deploy (branch `demo-website` merged, not force-pushed, so the wireframes at /a /b /c still resolve). Tests **226/226**, build green, all 10 routes walked in Chrome, 23-href crawl clean. Then **M4-b, the mobile pass** (founder reported mobile issues on the preview): the room reveals' ±46px X translate was making mobile Chrome widen the LAYOUT viewport, which dragged every `position: fixed` element wider and let the page pan sideways — sideways reveals now wait for 960px; plus wrapping ghost labels, PhoneFrame `max-w-full` + `min-w-0`, `p { text-wrap: pretty }` moved into `@layer base` (it was silently beating `truncate` on the guide dock), and **CompareTable stacks one card per claim below sm** instead of a 640px sideways scroll. Verified 320/360/390/430px: 9/9 routes clean, live-verified on the preview. **NEXT = M5** (delete retired components + `teal-deep`, full Lighthouse/axe/JSON-LD/SSR pass, then llms.txt + AI-crawler robots rules + last-updated dates), then the founder review of the preview. Founder-gated, never invent: REV-a hero art, REV-b subscription/camera/ship/languages, REV-c stale ₹2,999 index; **REV-d app images is now ASKABLE** (brief pointer 9 gated it on the v1 deploy, which has happened). Checkpoint: `docs/checkpoints/revamp-theme-b-2026-07-25.md`.
