...
- Visuals are the **immersive R3F redesign** (founder mandate 2026-07-08/09, REVERSED the earlier "2D everywhere" call): Home is one continuous 3D journey — fixed canvas behind SSR DOM (`site/components/three/`, `site/lib/three/`), founder-generated GLBs in `site/public/models/` (mascot = rigged clip-less, animated procedurally: Tripo presets break the chibi body), wash handoff via `html.scene-3d`, StageGate fallback gates (reduced-motion/no-JS/saveData get the flat site; phones keep DOM art). Plan + architecture: `docs/redesign-plan-2026-07.md`. `MascotScene.tsx` (2D parallax) is now the fallback/interior treatment. `site/public/video/launch.{mp4,jpg}` = the "Two friends" film (source `launch-video/src/FilmTwoFriendsVeo.tsx`)
