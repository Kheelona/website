...
- **Animation retarget issue (2026-07-09):** the Humanoid-typed rig crumples the chibi body under big human presets ("wait" collapsed the torso/head). Triage order: (1) preview **idle** free, it distorts least and worked on v2; (2) one rig **Retry** (20 credits) max; (3) if still broken, STOP: ship the clip-less rigged GLB (already verified) and the site does the idle procedurally (breathing bob + head-bone cursor follow, the old HeroScene fallback pattern). Do not chase the retarget with more credits.
