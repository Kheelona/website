...
- **Animation retarget issue (2026-07-09):** the Humanoid-typed rig crumples the chibi body under human presets (wait AND idle both broke). **FOUNDER DECISION: mascot ships as the clip-less rigged GLB; the site animates it procedurally.** Skeleton verified post-compression: 42 bones incl. `Head`, `NeckTwist01/02`, `Spine01/02`, `Hip` — Stage B drives Head (+ neck falloff) for cursor-follow and Spine/Hip for the breathing idle. `exports/kheelona-mascot-v3-{raw,1k,2k}.glb` are the final mascot artifacts.
- **REMAINING: Lumi plush** — founder generating now: Retopo ~20k with texture bake → texture check (regen route if degraded) → Export GLB, Pack UV ON, Bottom Center Pivot ON, NO rig/animation → `~/Downloads` → tell Claude.
