...
describe("AnswerBlock", () => {
  it("leads with the question as a heading and answers it in visible copy", () => {
    render(
      <AnswerBlock
        ...
        answer="Not all of them. Lumi wakes to a word and is off the rest of the time."
      />,
    );
    expect(
    ...
    expect(
      screen.getByText(/Lumi wakes to a word and is off the rest of the time/),
    ).toBeInTheDocument();
  });
  ...
  it("gives the answer a deep-link anchor that clears the sticky navbar", () => {
    const { container } = render(
      <AnswerBlock id="always-listening" question="Is Lumi always listening?" answer="No." />,
    );
