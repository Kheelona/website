# WORKING FILE, historical half (the 2026-07 revamp rounds)

Split out of `WORKING.md` on 2026-08-23, verbatim. That file is the one every resume reads first, and
340 of its 463 lines were the superseded record of finished rounds: three retired cold-restart blocks
and the theme-B revamp's own working notes. Nothing is edited here, so it stays quotable as evidence.

**⚠ THIS IS A RECORD, NOT LAW.** Several statements below were true when written and are now WRONG.
The ones that will mislead you fastest:

- **"ages 3 to 10"** appears in a section headed *Locked decisions*. Lumi is **2 to 5** and the
  platform arc is **2 to 14**. Both "3 to 6" and "3 to 10" are dead; render from `LUMI_AGES`.
- **The Tally form and its launch gate.** Retired 2026-08-22; pre-orders are paid now.
...
narrative that used to sit in `project-state.json` is in `docs/checkpoints/closed-rounds.md`.
...
**V5 (2026-07-31) IS MERGED AND LIVE** (`740845a` on `main`, both branches in sync; rollback tag `v4-live-2026-07-31`): the
