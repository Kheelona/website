...
- **Stack (locked)**: Next.js App Router + TypeScript, Tailwind CSS (tokens from `Design/design-system/`), Framer Motion (reduced-motion aware), MDX for Stories, Tally for pre-order, GA4 + Vercel Analytics, Vercel hosting. No live WebGL at launch; mascot = renders + parallax/tilt (R3F slot documented for future GLB).
...
6. **/stories**: index + 4 seed MDX articles ("Why three to six are the years that matter most", "Screen-free does not mean silent.", "How children learn by talking.", "What to look for in a safe AI toy."), soft pre-order close each. Article JSON-LD. KW: per keyword map.
...
| S7 | /team | Auto |
| S8 | /stories + 4 seed MDX articles | Auto |
