...
| V3 modes (2026-07-28) | The founder asked whether the site showed Lumi's three modes. It did not: conversation was the spine of every page but was never NAMED as a mode, and Bluetooth was one line in a six-card grid. New `LumiModes` organism replaced "Three ways to be there" and its Companion/Storyteller/Teacher chips (personality words, where a parent asks what it does): **Talk about anything / AI mode · Learn inside a story / Kheelu mode · Play your own music / Bluetooth mode**. Full cards on `/products/lumi#modes`, a compact strip closing Home's day-with-Lumi room, the duplicate Bluetooth card removed from the what-it-does grid, a "Can Lumi play music?" FAQ mirrored into schema, and the modes listed in `/llms.txt`. All three were already-published facts, so no new gate. Bluetooth is framed as a MODE, never as a subscription fallback (V3-b), with a test asserting that. |
...
| V3 QA | axe **zero violations** on 10 routes × desktop and mobile. Lighthouse **A11y/BP/SEO 100 on all 18 runs**, Perf 99–100 (mobile on devtools throttling). Tests **215/215**. |
