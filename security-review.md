...
### F-09 · LOW · HSTS lacks `includeSubDomains` and `preload` · APPROVED BUT BLOCKED BY F-15

`max-age=63072000` is present on both hosts. The founder said yes to `includeSubDomains` on
2026-08-23. **It was not enabled, because checking first turned up F-15.**

The check that mattered: `includeSubDomains` binds every subdomain, and this domain has four, two of
which are nothing to do with this repo. `www` and `store` are fine. `api.kheelona.com` is fine: it
redirects HTTP to HTTPS and its certificate validates. **`admin.kheelona.com` does not** (see F-15),
and HSTS makes a certificate warning impossible to click through, so enabling this would hard-block
that panel for two years per browser. Enabling it would have been doing what was asked and breaking
something at the same time.

Sequence: fix or remove `admin.kheelona.com`, then this is a one-line change to
`securityHeaders()` plus a line in `test/security-headers.test.ts`.

`preload` is deliberately still out of scope and needs its own decision: it means submission to
hstspreload.org, and removal from that list takes months.

### F-15 · MEDIUM · `admin.kheelona.com` has a certificate that does not cover it

Found while checking F-09's blast radius. `admin.kheelona.com` resolves to Firebase Hosting
(`singular-arcana-479108-v5.web.app`), redirects HTTP to HTTPS, and then presents a certificate whose
subject is `CN=firebaseapp.com` with no name matching the host, so `curl` refuses it and a browser
shows a full warning interstitial.

**Why it is a MEDIUM and not cosmetic.** It is named `admin`, so it is guessable, and whoever uses it
has to click through a TLS warning every time. That is the habit that makes a session interception
work, and on an admin panel the thing being intercepted is administrative access. Separately, a
warning page is indistinguishable from the warning an actual interception would produce.

**Not this repo, and not my DNS.** Fix the Firebase custom-domain certificate, or delete the record
if the panel is unused. **I did not probe the host beyond reading its certificate** — no requests to
its application, no login surface touched, nothing enumerated.
